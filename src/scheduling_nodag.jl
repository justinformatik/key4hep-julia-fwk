...
function run_pipeline(data_flow::DataFlowGraph;
    ...
    graphs_tasks = Dict{Int, Task}()
    notifications = Channel{Int}(max_concurrent)

    # run graph in a thread
    function run_graph(idx::Int)
        event = Event(data_flow, idx)
        terminating_results = schedule_graph!(event, crunch_coefficients)
        notify_graph_finalization(notifications, idx, terminating_results...)
    end
    ...
    for idx in 1:event_count
        ...
        task = Threads.@spawn run_graph(idx)
        graphs_tasks[idx] = task
        @info dispatch_begin_msg(idx)
    end
    for (idx, task) in graphs_tasks
        wait(task)
